...
//        var color;
//        var colorbg;
//        if(style) { color = style.color; colorbg = style.colorbg; }
//        var edge = { source: s, target: t, color: color, colorbg: colorbg };
...
//    this.mouse_in = false;
////    console.log(this.r);
////    this.r.canvas.mouseout(function (event) {console.log("MouseOver");});
//    
//    /*
//     * Dragging
//     */
...
////            console.log(newY, e.clientX,selfRef.isDrag.dx);
...
//                selfRef.r.connection(selfRef.graph.edges[i].connection);
//            }
//            //selfRef.r.safari();
    ...
    drawEdge: function(edge) {
        ...
        if(edge.connection) {
            for (var i in this.graph.edges) {
                this.r.connection(this.graph.edges[i].connection);
            }
            return;
        }
        edge.connection = this.r.connection(edge.source.shape, edge.target.shape, edge.color, edge.colorbg);
    }
};
Raphael.Layout = {};
Raphael.Layout.Spring = function(graph) {
                this.graph = graph;
                this.iterations = 500;
...
Raphael.Layout.Spring.prototype = {
        layout: function() {
                this.layoutPrepare();
